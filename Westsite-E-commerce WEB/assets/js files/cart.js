  ...
  element.forEach((el) => {
    ...
    const ProductData = ` 
                            <div class="cartProduct pt-4 d-block d-sm-flex  align-items-center  w-100">
                                <a href="description.htm?ID=${encodeURIComponent(el.id)}"><img src="${el.image}"
                                    alt="" height="150px"></a>
                                <section class="sec1 ps-0 ps-sm-3 mt-3 mt-sm-0">
                                    <h6>${el.vendor}</h6>
                                    <h5>${el.title}</h5>
                                    <h4 style="margin-top: 15px;font-size: 20px;">₹ ${el.price}</h4>
                                    <p>Delivery By 4-5 days</p>
                                </section>
                                <section class="sec2 d-flex justify-content-center mt-4 mt-sm-0">
                                    <button data-id="1" class="decreaseBtn" id=${ el.id }>-</button>
                                    <span style="font-size: 15px;font-weight: bold;">Qty : ${el.quantity} </span>
                                    <button data-id="1" class="increaseBtn text-dark" id=${el.id }>+</button>
                                </section>
                                <section class="sec2 mt-3 mt-sm-0">
                                    <h5 style="text-align: center;font-size:17px">₹  ${el.price * el.quantity}</h5>
                                </section>
                                <section class="sec3 d-flex justify-content-center">
                                    <button data-id="1" class="removeButton" id=${ el.id}><i class="ri-delete-bin-6-fill  removeButton" id=${ el.id}></i></button>
                                </section>
                                <hr>
                            </div>`;

              productDisplayContainer.innerHTML += ProductData;
              badgeValue.innerText =  totalQTY 
